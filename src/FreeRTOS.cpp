...
#define Yerr 4300      // Y轴摄像头与夹爪偏差
#define Xerr 800       // X轴摄像头与夹爪偏差
// 系统状态
enum SystemState
{
  STATE_IDLE,           // 空闲等待视觉数据
...
void action_task(void *xTask5)
{
    ...
    {
        // 接收队列中的消息指针（阻塞等待）
        if (xQueueReceive(command_to_action_Queue, &received_msg, portMAX_DELAY) == pdPASS)
        {
            if (received_msg == NULL) continue;
                    ...
                    switch (device_id) {
                        ...
                        case 2:   // 八连杆工作台
                            if (value == 0) {
                                ...
                                setPCA9685_Motor(0, 1000+value*10); // 八连杆
                            }
                            break;
                        ...
                        case 6:   // 传送带
                            if (value == 0) {
                                ...
                                // 例如：analogWrite(pin, map(value, 0,100, 0,255));
                            }
                            break;
  ...
  while (1)
  {
    ...
    if (systemEnabled)
    {
      ...
      case STATE_GRIPPING:
      {
        ...
        vTaskDelay(pdMS_TO_TICKS(500)); // 抓取需要更多时间
...
void FreeRTOS_Setup(void)
{
  ...
  // 应用核 Core 1 (核心1)
  xTaskCreatePinnedToCore(sensor_task, "sensor_task", 4096, NULL, 2, &sensor_taskHandle, 1);
  xTaskCreatePinnedToCore(action_task, "action_task", 4096, NULL, 4, &action_taskHandle, 1);
  xTaskCreatePinnedToCore(command_task, "command_task", 4096, NULL, 3, &command_taskHandle, 1);

  // 处理核Core 0
  xTaskCreatePinnedToCore(vision_task, "vision_task", 4096, NULL, 1, &vision_taskHandle, 1);
